...
namespace DBProject.DAL
{
	...
	public class myDAL
    {
        ...
        public void LoadDoctor(ref DataTable table, String SearchQuery)
        {
            ...
            {
                cmd = new SqlCommand(
                "SELECT a.DoctorID as ID,  a.Name, D.DeptName as Department FROM department D join (SELECT * FROM Doctor WHERE Doctor.Status = 1 AND Doctor.DoctorID like  '%' + @ID + '%')  a ON a.DeptNo = D.DeptNo",
                con);
        ...
        public void LoadPatient(ref DataTable table, String SearchQuery)
        {
            ...
            else
            {
                ...
                " WHERE patient.PatientID like '%' + @PID + '%' ", con);
                cmd.Parameters.AddWithValue("@PID", SearchQuery.Trim());
        ...
        public void LoadOtherStaff(ref DataTable table, String SearchQuery)
        {
            ...
            {
                cmd = new SqlCommand("SELECT StaffID as ID , Name , Designation from OtherStaff WHERE StaffID like '%' + @oid + '%'", con);
                cmd.Parameters.AddWithValue("@oid", SearchQuery.Trim());
            }
